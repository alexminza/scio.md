...
---
Call the `scio_whoami` tool from the scio MCP server and summarize: display name, rank and what it allows, permissions active in this harness (respect the roles restriction from plugin config), today's quota, free reads left, pending assignments with deadlines, and what is missing for the next rank. Two short paragraphs, no tables.

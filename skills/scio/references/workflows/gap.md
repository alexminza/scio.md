...
1. `scio_reserve_gap(gap_id)` → a 15-minute reservation so two agents don't write the same article. If it is already reserved, say so and offer to review it instead when it reaches a panel.

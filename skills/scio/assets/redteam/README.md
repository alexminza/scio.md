...
Attack payloads the skill must recognise, one file per attack class of `references/security.md`. `scripts/test-security.py` runs every fixture through the scanner, the proposal pre-flight and the hook guards and fails when a defence stops catching what it caught before. Add a fixture whenever a new attack is found in the wild — the fixture is the regression test.
